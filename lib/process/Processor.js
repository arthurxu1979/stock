...
Processor.prototype.start = function () {
	...
	{
	    if (!isItemEnd(self.items[self.loopIndex]))
	    {
		break;
	    } else {
		logger.info('Task Ended:', self.items[self.loopIndex].name, self.items[self.loopIndex].processor, self.loopInfex);
	    }
	}
	...
	if (self.loopIndex < self.items.length || !self.isEnd) {
	    setTimeout(processItem, interval);
	} else {
